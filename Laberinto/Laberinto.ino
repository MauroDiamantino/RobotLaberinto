...
#define DEBUG 1
#define LAZO_A_CALIBRAR 'R' //debe ser 'R' o 'L'
...
const unsigned int INT_CAMBIO_PARAM = (unsigned int) 5000/(1000*INT_MUESTREO); //Intervalo de tiempo (en mS) para el cambio de los parametreos de los 
                                             //controldadores PID. El micro debe revisar el buffer de entrada serie cada vez que se cumple este intervalo de 
                                             //tiempo y actualizar los parametros de los PIDs si se han enviado nuevos valores. 5mS.        
...
const float ZONA_MUERTA_R = 3.0; //Zona muerta del motor derecho (en volts), solo considerando tensiones positivas. En realidad la zona muerta es ±4v.
const float ZONA_MUERTA_L = 3.0; //Zona muerta del motor derecho (en volts), solo considerando tensiones positivas. En realidad la zona muerta es ±4v.
...
//Estructura que permite sacar ciertas variables de interes de las funciones de Control, para transmitirlas por el puerto serie.
...
const float kp_l = 8.0, ki_l = 1.7, td_l = 0.08; //Parametros del PID
float r_left = 0.0; //Velocidad lineal de referencia (m/s)

//Parametros del PID del lazo de control L y referencia
const float kp_r = 8.0, ki_r = 1.5, td_r = 0.12; //Parametros del PID
...
void setup(){
  ...
  setPwmFrequency(PIN1_MOTOR_LEFT, 8); //se configura la freq PWM en 4KHz aprox
  setPwmFrequency(PIN2_MOTOR_LEFT, 8); //se configura la freq PWM en 4KHz aprox
  setPwmFrequency(PIN1_MOTOR_RIGHT, 8); //se configura la freq PWM en 8KHz aprox
  setPwmFrequency(PIN2_MOTOR_RIGHT, 8); //se configura la freq PWM en 8KHz aprox
...
//          Serial.println(respControladorL.u_p);
          sprintf(bufferOut,"%d,%d",int(100*r_left),int(100*respControladorL.y));
...
//          Serial.println(respControladorR.u_p);
          sprintf(bufferOut,"%d,%d",int(100*r_right),int(100*respControladorR.y));
          Serial.println(bufferOut);
        #endif
    #endif
...
inline float Actuador(float u, const unsigned int PIN1_MOTOR, const unsigned int PIN2_MOTOR, const float ZONA_MUERTA){
  ...
  if (u >= 0.0){ //voltaje medio de alimentacion del motor positivo
    //Control de la zona muerta y saturacion del actuador
    if ( u!=0.0 && u<ZONA_MUERTA ){  
      u = ZONA_MUERTA; 
    }else if (u > TENSION_MAX){
      u = TENSION_MAX;  
    }
...
inline RespuestaControlador ControlMotorRight(float r){
  ...
  const byte N = 2; //Razon entre la constante de tiempo de la accion derivativa y la constante de tiempo del filtro en cascada.
  const byte N_filt = 20;
  //Variables del lazo de control R
  float y; //velocidad lineal sensada (m/s)
  static float reg_filt[N_filt];
  float sum=0.0;
  float y_filt;
  float e; //error de velocidad (m/s)
  float u, u_p; //accion de control, voltaje medio a aplicar en el motor, que se materializa
...
//#endif
  ...
  //genera a partir de la salida de lazo (y). Ademas se implementa un filtro pasa-bajo en cascada con la accion derivativa.
  //Termino integrativo, con aproximacion rectangular y con la tecnica antiwindup "back-calculation"
  integral = integral1 + INT_MUESTREO*(kp_r*ki_r*e1 + antiwindup); //El valor de la variable "antiwindup" corresponde al intervalo de muestro anterior
  //integral = integral1 + kp_r*ki_r*INT_MUESTREO*(e + e1)/2.0;
  //Termino derivativo (se implementa con un filtro pasa-bajo en cascada)
  deriv = td_r/(N*INT_MUESTREO + td_r) * (deriv1 - kp_r*N*(y_filt - y_filt1));
  //Termino proporcional
  prop = kp_l*e;
  //Accion de control total
  u = prop + integral + deriv;
  ...
  //Aplicacion de la señal de control generada por el PID
  u_p=Actuador(u, PIN1_MOTOR_RIGHT, PIN2_MOTOR_RIGHT, ZONA_MUERTA_R);
